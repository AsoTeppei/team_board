...
## 社員マスター一元化ポリシーとの関係 (2026-04-22)

本アプリは Google Sheets / GAS で独自にメンバー管理しており、`YonekuraSystemDB.dbo.Users` とは連携していないため、**社員マスター一元化ポリシー (YonekuraPortal 一元管理) の直接対象外**。ただし将来、米倉社内管理ポータル (YonekuraPortal) に繁忙状況を表示するなどの連携を加える場合は、ポータルの社員 ID (`Users.UserID`) をキーに照合する方針で進める。

- 本アプリ単独で閉じているうちはメンバー情報を GAS / スプレッドシートに持ってよい
- 新規に `YonekuraSystemDB.dbo.Users` を参照する機能を追加する場合は、**そこで社員を新規登録・基本情報編集しない** (ポータル側で行う)
